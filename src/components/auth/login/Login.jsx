...
const Login = () => {
  ...
  return (
    <div style={BG_STYLE_LOGIN} className="min-h-screen flex items-center justify-center">

      <form className="bg-gray-200 p-8  rounded-xl shadow-2xl m-auto" onSubmit={handleSubmit}>
        <h2 className="text-2xl font-bold mb-6 text-center">Login</h2>

        <TextInput required={true} label="Name" customClassName="md:w-full mb-5" value={name} changeHandler={(e) => setName(e.target.value)} placeholder="Enter your name" />

        {/* Phone number input using MultiInputField */}
        <div className="mb-6">
          <label className="block mb-2 text-gray-700">Phone Number</label>
          <MultiInputField defaultValue={phoneNumber} totalFields={10} onChange={handlePhoneNumberChange} />
        </div>
       
        <SuccessButton label="Continue"  type="submit" actionHandler={handleSubmit} customClassName="w-full bg-green-800 hover:bg-green-900 text-white"/>
      </form>
    </div>
  );
};
